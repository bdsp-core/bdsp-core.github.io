...
// `path` is the dense sweep polyline ([mu2,mu1,nu] points) generated by the
// chosen path type (arc / circle / piecewise). The number of points sets the
// sweep speed, so the path generator bakes in the transition-speed k.
export function simulate(path, opts) {
  const {
    fs = 200, dur = 16, tstep = 0.02, freq = 10,
  ...
  // path (carrying state) instead of probing points in isolation.
  const arc = path.map(onSphere);                       // ensure on the sphere
  const onset = arc[0];
  const offset = arc[arc.length - 1];
